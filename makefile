...
ALL_CFLAGS := $(ALL_RELEASE) $(DEFS)  $(CFLAGS)
ALL_CXXFLAGS := $(ALL_RELEASE) $(DEFS) $(INCLUDES) $(CFLAGS)
LDFLAGS :=

$(CC) $(ALL_CFLAGS)

capturevpedisplay: capturevpedisplay.cpp vip_obj.cpp vpe_obj.cpp cmem_buf.cpp save_utils.cpp disp_obj.cpp
	$(CC) $(ALL_CFLAGS) -o $@ $^  -lpthread -lticmem -lopencv_highgui -lopencv_imgcodecs -lopencv_imgproc -lopencv_core $(INCLUDES) -ldrm  -ldrm_omap
